...
class SaveSynthesisPaths(MetricsBase):
    def __init__(
        self, run_dir: str, proxy_component_names: List[str], file_name: str = "paths.csv"
    ):
        super().__init__()
        self.path = Path(run_dir) / file_name
        print("Saving synthesis paths to ", self.path)
        self.unique_molecules = set()
        self.proxy_component_names = proxy_component_names
        with open(self.path, "w") as f:
            if len(proxy_component_names) == 0:
                f.write("iteration,path,molecule,proxy\n")
            else:
                components_str = ",".join(proxy_component_names)
    ...
    def compute_metrics(self, trajectories_container: TrajectoriesContainer) -> Dict[str, float]:
        ...
        components_dict = {
            k: reward_outputs.proxy_components[k] for k in self.proxy_component_names
        }
        ...
        for i, (states, actions, score) in enumerate(zip(states_grouped, actions_grouped, proxy)):
            ...
            if len(self.proxy_component_names) == 0:
                to_be_added.append(
